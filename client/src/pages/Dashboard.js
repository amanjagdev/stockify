...
const Dashboard = () => {
  ...
  const handleAdd = () => {
    if (stockName === '' || stockQty === 0 || stockPrice === 0 || thisStockOwner === '') {
      ...
      Axios.post(
        `${process.env.REACT_APP_API_URL}/stocks/create`,
  ...
  const removeItem = _id => {
    Axios.post(
      `${process.env.REACT_APP_API_URL}/stocks/delete`,
      { user: user.user, _id },
      {
        headers: {
          Authorization: 'Bearer ' + user.token,
  ...
  const addWareHouseNumber = () => {
    Axios.post(
      `${process.env.REACT_APP_API_URL}/stocks/addwarehouse`,
      {
        user: user.user,
        warehouseNo: whno,
      },
  ...
  const saveNewItem = _id => {
    Axios.post(
      `${process.env.REACT_APP_API_URL}/stocks/update`,
      {
        user: user.user,
        _id,
        name: newStockName,
    ...
    <div className="Dashboard">
      ...
      <div className="warehouse-qunatity">
        {!user.user.warehouseNo && (
          <div className="set">
            ...
            <button className="rev" onClick={() => addWareHouseNumber()}>
              Set Quantity
            </button>
          </div>
        )}
                ...
                return (
                  <React.Fragment key={_id}>
                    <tr>
                      ...
                      <td>
                        <button onClick={() => editItem(_id, name, quantity, stockOwner, price)}>
                          <img src={require('../assets/edit_icon.png')} alt="Edit" />
                        </button>
                      </td>
                      <td>
                        <button onClick={() => removeItem(_id)}>
                          <img src={require('../assets/delete_icon.png')} alt="delete" />
                        </button>
                      </td>
                    </tr>
                    <tr className={currentlyEditing === _id ? 'yes' : 'no'}>
                      <td>
                        <input
                          type="text"
                          className="stock-name-box"
                          value={newStockOwner}
                          onChange={e => setNewStockOwner(e.target.value)}
                        />
                      </td>
                      <td>
                        <input
                      ...
                      <td>
                        <button className="rev save" onClick={() => saveNewItem(_id)}>
                          Save
                        </button>
                      </td>
                    </tr>
      ...
      <div className="add-form">
        ...
        <input
          type="text"
          id="stockName"
          value={thisStockOwner}
          onChange={e => setThisStockOwner(e.target.value)}
        />
      ...
      <button className="add rev" onClick={() => handleAdd()}>
        Add
      </button>
      {errors &&
